...
import { softDeleteSitePageRuntime } from '@/features/ecommpanel/server/siteBuilderStore';
...
export async function DELETE(req: NextRequest, context: { params: Promise<{ pageId: string }> }) {
  ...
  try {
    const page = await softDeleteSitePageRuntime(pageId);
    if (!page) return errorNoStore(404, 'Rota não encontrada.');

    return jsonNoStore({ ok: true, route: page });
  } catch (error) {
    return errorNoStore(503, error instanceof Error ? error.message : 'Não foi possível mover a rota para a lixeira.');
  }
}
