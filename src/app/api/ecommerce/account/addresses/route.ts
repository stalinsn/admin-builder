...
import { upsertCustomerAddress } from '@/features/ecommerce/server/customerAccountStore';
...
export async function POST(req: NextRequest) {
  ...
  const rate = checkRateLimit(
    `customer:address:create:${getRequestFingerprint(req)}`,
  ...
  if (!address) {
    return errorNoStore(400, 'Não foi possível salvar o endereço.');
  }
